...
import copy
import argparse
from nets.models import ClipModel_visual
...
from utils.clip_util import compute_local_mean_insight, aggregate_global_insight_matrix
...
def main(Location, dataset, method):
    ...
    parser.add_argument('--method', type=str, default='FedDIM')
    ...
    server_model = ClipModel_visual(
        args.net, num_classes=args.num_classes, freezepy=True)
    train_loaders, val_loaders, test_loaders, test_train, train_test_loaders, Labels = get_data(
        args.dataset)(args, server_model)
    server_model.initdgatal(test_loaders[4])
    ...
    optimizers = [optim.AdamW(
        params=[{'params': models[idx].model.parameters(), 'lr': args.lr, 'betas': (args.beta1, args.beta2)},
                {'params': models[idx].projector.parameters(), 'lr': args.lr * 10., 'betas': (args.beta1, args.beta2)}],
    ...
    for a_iter in range(args.iters):  # All Epoch
        ...
        for wi in range(args.wk_iters):  # Each client local training epoch
            ...
            for client_idx, model in enumerate(models):
                if client_idx in args.test_envs:
                    ...
                    if a_iter == 0:
                        buffer.append(compute_local_mean_insight(model,train_test_loaders[client_idx], device, args.num_classes))
                        train(
                            ...
                            mlp[client_idx], target_prototypes=buffer[client_idx])

                    if a_iter != 0:
                        train(
                            args, model, train_test_loaders[client_idx], optimizers[client_idx], device,
                            test_train[0], adv[client_idx], server_model_pre, previous_nets[client_idx],
                            mlp[client_idx], target_prototypes=global_insight_proto)
                        buffer.append(compute_local_mean_insight(model, train_test_loaders[client_idx], device, args.num_classes))
        ...
        with torch.no_grad():
            ...
            global_insight_proto = aggregate_global_insight_matrix(
                buffer,
                previous_global_proto=global_insight_proto,
                momentum=0.7
            )
            ...
            if (sum(Val_Acc)) / sclient_num > Val_mean:
                ...
                for i in range(0, sclient_num):
                    ...
                    torch.save(models[i].projector.state_dict(),
                               Location + 'projector' + str(i) + '.pth')

    # Testing
    with torch.no_grad():
        for i in range(0, sclient_num):
            models[i].model.load_state_dict(torch.load(Location + 'Client' + str(i) + '.pth'))
            models[i].projector.load_state_dict(torch.load(Location + 'projector' + str(i) + '.pth'))
        server_model, models = communication(
            args, server_model, models, client_weights)
...
if __name__ == '__main__':
    ...
    Location = ('./SavedModel/OfficeHome/IJCAI2026/ACPR/'
                '/FedDIM/ViT-B-32/lr=5e-5/')
    method = '/FedDIM/ViT-B-32/lr=5e-5/'
    dataset = '/OfficeHome/IJCAI2026/ACPR/'
