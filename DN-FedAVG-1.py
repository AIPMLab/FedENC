...
def main(Location, dataset, method):
    ...
    for client_idx in range(client_num):
        ...
        if client_idx in args.test_envs:
            client_feature_counts.append(0)
            pass
            ...
            models[client_idx].model.to(device)
            test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none, all_f = Glotest(args, models[client_idx],
                                                                                      test_loaders[client_idx],
                                                                                      device)
            models[client_idx].model.to('cpu')
            models[client_idx].fea_attn.to('cpu')
            ...
            client_feature_counts.append(all_f.shape[0])

    tsne = TSNE(n_components=2, random_state=42, perplexity=40, init='pca')
    all_features = np.concatenate(all_features, axis=0)
    tsne_results = tsne.fit_transform(all_features)
    save_dir = './tSNEResults/OfficeHome/IJCAI2026/FedAVG/'
    ...
    for client_idx in range(client_num):
        if client_idx in args.test_envs:
            continue
        count = client_feature_counts[client_idx_iter]
        if count > 0:
            client_tsne = tsne_results[start:start + count]
            loc = os.path.join(save_dir, f'{client_idx}_2DtSNE.csv')
            np.savetxt(loc, client_tsne, delimiter=',', fmt='%.6f')
            start += count
        client_idx_iter += 1
...
if __name__ == '__main__':
    ...
    Location = ('./SavedModel/OfficeHome/IJCAI2026/ACPR/'
                '/FedAVG/ViT-B-32/lr=5e-5/')
    method = '/FedAVG/ViT-B-32/lr=5e-5/'
