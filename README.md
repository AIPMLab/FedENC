...
```sh
python xxx.py (e.g., python Ours.py
```

### Ours.py (example)

Run Ours.py to simulate our algorithm. For convenience, we split all methodologies individual files, each file can be used for only one Method.
...
clip_util.py is the utils that CLIP will use. For FedAVG and LoRA, you have to do the following steps:
```sh
def freeze_param(model):
    for name, param in model.named_parameters():
        param.requires_grad = True
```
For ours, FedCLIP, PromptFL, CocoOP, FedAPT abd LP++, you have to set it as False.
