...
def main(Location, dataset, method):
    ...
    parser.add_argument('--dataset', type=str, default='ImageNetBG')
    ...
    parser.add_argument('--test_envs', type=int, nargs='+', default=[30])
    ...
    parser.add_argument('--method', type=str, default='normfit')
    ...
    server_model.initdgatal(test_loaders[30])
        ...
        with torch.no_grad():
            ...
            for client_idx in range(client_num):
                if client_idx in args.test_envs:  # Global no validation part
                    ...
                    models[client_idx].model.to(device)
                    test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none, _ = Glotest(args,
                                                                                              models[client_idx],
    ...
    for client_idx in range(client_num):
        ...
        if client_idx in args.test_envs:
            ...
            models[client_idx].model.to(device)
            test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none, _ = Glotest(args, models[client_idx],
                                                                                      test_loaders[client_idx],
                                                                                      device)
            models[client_idx].model.to('cpu')
            models[client_idx].fea_attn.to('cpu')
...
if __name__ == '__main__':
    ...
    Location = ('./SavedModel/ImageNetBG/IJCAI2026/'
                '/NormFit/ViT-B-32/lr=5e-4/')
    method = '/NormFit/ViT-B-32/lr=5e-4/'
    dataset = '/ImageNetBG/IJCAI2026/'
